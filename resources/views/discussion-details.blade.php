...
<link rel="preload" href="{{ asset_min('assets/css/discussion-details.css') }}" as="style" onload="this.onload=null;this.rel='stylesheet'">
<noscript><link rel="stylesheet" href="{{ asset_min('assets/css/discussion-details.css') }}"></noscript>
@endpush
      ...
      <div class="sidebar-card book-mini-card">
        ...
        <a href="{{ route('book-details', $discussion->book->slug) }}" class="book-mini-link">
          @if ($discussion->book->cover_image)
            <img src="{{ $discussion->book->cover_image_sm_url }}" width="174" height="285" loading="lazy" decoding="async" alt="{{ $discussion->book->title }}" class="book-cover mini" style="padding:0;width:70px;height:90px;object-fit:cover;">
          @else
